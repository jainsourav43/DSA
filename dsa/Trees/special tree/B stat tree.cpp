...
void redistribution1(bptr &t,int a[],int s,bptr lc,bptr rc)
{
 ...
 void redistribution3(bptr &t,int a[],int s,bptr lc,bptr rc,int ch)
 {
 	int temp,i,j,b[2*d+1];
 	if(ch>s)
 	{
 		for(i=s;i<ch;i++)
 		{
			...
			temp=t->par->key[i];
			t->par->key[s]=a[2*d];
			if(t->par->ptr[i+1]->count==2*d-1)
			{
			for(j=t->par->ptr[i+1]->count-1;j>=0;j++)
			{
			b[j+1]=t->par->ptr[i+1]->key[j];
			t->par->ptr[i+1]->key[j+1]=t->par->ptr[i+1]->key[j]
			}
			b[t->par->ptr[i+1]->count+1]=temp;
			sort(b,b+5);
			t->par->key[s]=b[2*d];
			t->par->ptr[i+1]->key[0]=temp;
			t->par
		    }
		   else
		   {
		   	for(j=t->par->ptr[i+1]->count;j>=0;j--)
		   	{
		   		t->par->ptr[i+1]->key[j+1]=t->par->ptr[i+1]->key[j];
		   		t->par->ptr[i+1]->ptr[j+2]=t->par->ptr[i+1]->ptr[j+1];
			}
			t->par->ptr[i+1]->ptr[1]=t->par->ptr[i+1]->ptr[0];
			t->par->ptr[i+1]->key[0]=temp;
			t->par->ptr[i+1]->count++;
			t->par->ptr[i+1]->ptr[0]=t->ptr[t->count+1];
		   }
		   for(j=2*d;j>=2;j--)
		    t->ptr[i]=t->ptr[i-1];
		    t->ptr[0]=lc;
	...
	{temp=t->par->key[s-1];
		...
		t->count=2*d-1;
		t->par->key[i-1]=a[0];
		t->par->ptr[i-1]->ptr[t->par->ptr[s-1]->count+1]=t->ptr[0];
		t->par->ptr[i-1]->key[t->par->ptr[i-1]->count+1]=temp;
		t->par->ptr[i-1]->count++;
	    }
	    else
	    {
	    	for(j=0;j<2*d;j++)
	    	b[j]=t->par->ptr[i-1]->key[j];
	    	b[2*d]=t->par->key[s-1];
  ...
  void redistribution2(bptr &t,int a[],int s,bptr lc,bptr rc)
 {
    ...
    t->par->ptr[s+1]->ptr[1]=t->par->ptr[s+1]->ptr[0];
