...
void redistribution1(bptr &t,int a[],int s,bptr lc,bptr rc,int k)
{
 ...
 void redistribution2(bptr &t,int a[],int s,bptr lc,bptr rc,int k)
 {
 		int temp;
    temp=t->par->key[s];
	int i;
...
void delete_node(bptr &t1,int k,int flag)
{
	...
	{
		cout<<"special \n";
		int co=t1->count;	int s=search1(t1,k);
		t1=node_combine1(t->ptr[in],t->ptr[in+1]);
	cout<<"k ="<<k<<endl;
		cout<<t->ptr[in+1]->key[0]<<"kjhg"<<"co ="<<co<<"s ="<<s;	
		if(s==co+1)
		{cout<<"90391\n";
		for(i=0;i<d;i++)
		t1->ptr[i+d+1]=t->ptr[in+1]->ptr[i];
		t1->ptr[2*d]=t->ptr[in+1]->ptr[d];
	    }
		t1->par=null;
	...
	{    cout<<"sssppp\n";
		...
		{ cout<<"sssppp  2\n";
		for(i=0;i<d;i++)
		node->ptr[i+d+1]=t->ptr[in+1]->ptr[i];
		node->ptr[2*d]=t->ptr[in+1]->ptr[d];
	    }
	    //node->ptr[3]=t->ptr[in+1]->ptr[0];
		if(in!=t->count)
		{
	    for(i=in;i<t->count;i++)
	 	{
...
if(s>0&&s<=3)
{
	...
	{
		redistribution1(t,a,s,lc,rc,k);
		return ;
	}
	else if(t->par->ptr[s+1]!=null&&t->par->ptr[s+1]->count<2*d-1)
	{
		redistribution2(t,a,s,lc,rc,k);
		return ;
	}
}
else if(s==0||s==4)
...
int main()
{
	...
	int n=1;
	t->par=null;
	int input[]={4,7,22,21,35,12,23,39,16,32,29,46,28,43,64,9,49,53,40,42,24,60,26,27,91,93,94,95,92,90,100,99};
