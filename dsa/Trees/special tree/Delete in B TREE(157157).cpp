...
int search2(bptr t,int k)
{
	...
	if(t!=null)
	{
		for(i=0;i<=t->count;i++)
		{
			...
			{
				return search2(t->ptr[i],k);
			}
			else if(t->key[i]<k&&i!=t->count&&(t->key[i+1]<k))
			{
				continue;
			}
			else if(t->key[i]<k&&i!=t->count&&(t->key[i+1]>k))
			{
				return search2(t->ptr[i+1],k);
			}
			else if(t->key[i]<k&&i==t->count)
			{
				return search2(t->ptr[i+1],k);
			}
		}
