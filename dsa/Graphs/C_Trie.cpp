...
void insert(trptr &tr1,char str[],char str1[],int n,int ind,int ke)
{
	...
	if((tr->ptr.ptr1[in]==null&&check==0)&&(tr->count==0)||check1==1)
	{
		cout<<"if\n";
		if(check1==0)
		{
	...
	else if((tr->ptr.ptr1[tr->count-1]==null&&check==0)&&(tr->count!=-1))
	{
	    if(check1==0)
		{
		leafnode *l;
	...
	else if(tr->ptr.ptr1[in]&&tr->ptr.ptr1[in]->tag==0)
	{
	    ...
	    {
	    	in=index(str,str1,tr->count);
	    	if(tr->ptr.ptr1[in]==null)
	    	{
	    	tr->ptr.ptr1[in]=new trnode;
			tr->ptr.ptr1[in]->par=tr;
			tr->ptr.ptr1[in]->tag=0;
			leafnode *l=new leafnode;
			strcpy(l->key,str1);
			tr->ptr.ptr1[in]->ptr.ptr2=l;
			}
		}
	}
	else
