...
void printlevel(bptr t)
{
	...
	while(q.front!=-1)
	{
		...
		{
			if(t2->key[i]==18&&t2->par)
			{
		//	cout<<"love = "<<t2->par->key[0]<<endl;
		    }
			cout<<t2->key[i]<<"  ";
		}
		cout<<endl;
...
void overflow(bptr &t1,bptr lc,bptr rc,int k,int a[])
{
	...
	{cout<<"OVERFLOW1\n";
	  ...
	  else
	  {
		...
		for(i=0;i<=d;i++)
		{
			if(t2->key[i]!=k)
			{
				...
				{   cout<<"i= "<<i<<endl;
					t4->ptr[i]=t->ptr[i];
					if(t->ptr[0])
					cout<<"ahead = "<<t->ptr[0]->key[0]<<" ahead  "<<t2->ptr[0]->key[0]<<endl;
						if(t2->ptr[i]&&t->ptr[1]!=null)
					{
						t2->ptr[i]->par=t4;
					}
		...
		//t->key[0]=a[d];
		if(t->ptr[0])
		cout<<"ahead = "<<t->ptr[0]->key[0]<<" ahead  "<<t2->ptr[0]->key[0]<<" ahead r ="<<t2->key[0]<<endl;
		if(t2->ptr[0])
		cout<<" t2->ptr[0]->key[0]= "<<t2->ptr[0]->key[0]<<endl;
		if(t2->ptr[0])
		cout<<"lki = "<<t2->key[0]<<endl;
		t->ptr[0]=t2;
		t2->ptr[0]=temp;	
		if(t->ptr[0])
		cout<<"ahead = "<<t->ptr[0]->key[0]<<endl;
		if(t2->ptr[0])
		cout<<" t2->ptr[0]->key[0]= "<<t2->ptr[0]->key[0]<<endl;
		cout<<"t2->key[0] ="<<t2->key[0]<<endl<<"t2->count ="<<t2->count<<endl;
		if(t2->ptr[0])
		cout<<"lki = "<<t2->ptr[0]->key[1]<<endl;
		t->ptr[1]=t3;	cout<<"t3->key[0] ="<<t3->key[0]<<endl;
		if(t3->ptr[0])
		cout<<" t3->ptr[0]->key[0]= "<<t3->ptr[0]->key[0]<<endl;
		t->key[0]=a[d];
		//cout<<"t2->key[0] ="<<t2->key[0]<<endl;
		//cout<<"t3->key[0] ="<<t3->key[0]<<endl;
	...
	else 
	{
		...
		//cout<<"t->par->par = "<<t->par->par<<endl;
		cout<<"t->par->key = "<<t->par->key[0]<<endl;
		bptr t2=new bnode;
		bptr t3=new bnode;
		initialise(t2);
		initialise(t3);
	  ...
	  if(t->ptr[0])
		{
		cout<<"hate 2="<<t->ptr[0]->key[1]<<endl;
		cout<<"hate story = "<<t->ptr[0]->par->key[0]<<endl;
	   }
		for(i=0;i<=d;i++)
		{
			if(t2->key[i]!=k)
		...
		if(t->ptr[0])
		{
		cout<<"hate 1="<<t->ptr[0]->key[1]<<endl;
		cout<<"hate story = "<<t->ptr[0]->par->key[0]<<endl;
	   }
		if(che==0&&che1==0)
		{
		...
		//if(t3->par)
		//cout<<"t3->par->key = "<<t3->par->key[0]<<endl;
		if(t->ptr[1]==null)
		{
		if(t->ptr[0])
		{
		cout<<"hate ="<<t->ptr[0]->key[1]<<endl;
		cout<<"hate story = "<<t->ptr[0]->par->key[0]<<endl;
	   }
		t2->ptr[0]=t->ptr[0];
		if(t->ptr[0])
...
int main()
{
	...
	cin>>n;
	int input[]={52,36,24,58,96,12,15,18,17,16,14,102,321,123,28,2,3,27,30,32,38,40,23,22};
	t->key[0]=n;
	t->count++;
	t->par=null;n--;
	while(n!=0)
	...
	cin>>k;
	cout<<"Reverse order printing before he searched key is \n";
	searchleaf(t,k);
	cout<<endl;
